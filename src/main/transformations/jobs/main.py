...
logger.info(f"List all files present at my local directory after download {all_files}")
...
# Note: Active means there is error in process and Inactive means process ran successfully
...
if correct_files:
    for files in correct_files:
        ...
        insert_statemets.append(statements)
    logger.info(f"Insert statement created for staging table --- {insert_statemets}")
    logger.info("************ Connecting with MySQL server **************")
    connection = get_mysql_connection()
    cursor = connection.cursor()
    logger.info("************ MySQL server connected successfully ************")
    for statement in insert_statemets:
        cursor.execute(statement)
        connection.commit()
    cursor.close()
    connection.close()
else:
    logger.error("************ There in no files to process ************")
...
final_df_to_process = spark.createDataFrame(data=temp_data, schema=schema)
# Create new column with concatenated value of extra columns 
...
logger.info("************* Final data mart for customer Data Mart *************")
